...
class Crawler:
    ...
    def polite_request(self, url,time_out = 60):
        ...
        while True:
            ...
            try:
                ...
                logging.error("Network Disconnect!!!")
                time_count += 5
                time.sleep(5)
            except requests.exceptions.RequestException:
                print(f"Request URLs {url} Error!!!")
                logging.error(f"Request URLs {url} Error!!!")
                time_count += 5
                time.sleep(5)
    ...
    def get_totalpage_totalproducts(self, url):
        ...
        try:
            ...
            logging.error(f'Not found total_page total_product ai {url} ')
    ...
    def get_infor(self, url):
        ...
        if response and response.status_code == 200:
            ...
            list_product_desc = [element.text.strip() for element in product_items.find_all('span', class_='short-description')]
            return list(zip(list_product_name, list_product_image,list_product_desc,list_product_price,list_product_link))
        else:
        ...
        if total_page and total_product:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_woker) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    data = future.result()
                    images_dowload = [(item[0],item[1]) for item in data if item[1] not in image_visited]
                    image_visited.update(images_dowload)
                    self.image_visited.update(images_dowload)
...
if __name__ == "__main__":
    urls_path = os.path.join(os.getcwd(),'product_links','urls_test.csv')
    ...
    crawler.run()
    end_time = time.time()
    logging.info(f'Finish crawl {len(crawler.image_visited)} product trong {end_time - start_time} s')
